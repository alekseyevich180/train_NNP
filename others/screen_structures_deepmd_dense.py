...
def run_aimd_presampling(
    ...
    def sample_frame() -> None:
        ...
        energy_raw = float(frame.get_potential_energy())
        ...
        if step <= 1000:
            ...
            BFGS(relaxed, logfile=None).run(fmax=0.5, steps=30)

            energy_relaxed = float(relaxed.get_potential_energy())

            filename = f"step_{step:06d}_relaxed.cif"
            write(str(sampled_dir / filename), relaxed)

            deepmd_writer.add_frame(relaxed, step)

            sampled_frames.append(relaxed)
            ...
            sampled_rows.append(
                {
                    ...
                    "delta_E": energy_relaxed - energy_raw,
                    "max_force": max_force,
                    "file": filename,
                }
            )
            ...
            if energy_raw - ref_energy > 2.0:
                return

            filename = f"step_{step:06d}.cif"
            write(str(sampled_dir / filename), frame)

            deepmd_writer.add_frame(frame, step)
    ...
    if sampled_rows:
        pd.DataFrame(sampled_rows).to_csv(sampled_dir / "sampling_stability.csv", index=False)
